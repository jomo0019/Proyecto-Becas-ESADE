//Notes:
//  Slighly modified version of the original file by Brian Kovak
...
capture program drop DCdensity_m
program DCdensity_m, rclass
{
  version 9.0
  set more off
  pause on
        ...
        (line `cellsmname' `evalname' if `evalname' > `breakpoint', lcolor(black) lwidth(medthick))   ///
          (line `hi' `evalname' if `evalname' < `breakpoint', lcolor(black) lwidth(vthin))              ///
            (line `lo' `evalname' if `evalname' < `breakpoint', lcolor(black) lwidth(vthin))              ///
              (line `hi' `evalname' if `evalname' > `breakpoint', lcolor(black) lwidth(vthin))              ///
                (line `lo' `evalname' if `evalname' > `breakpoint', lcolor(black) lwidth(vthin)),             ///
                  xline(`breakpoint', lcolor(black)) legend(off) graphregion(color(white)) xtitle("Distance to the income threshold (euros)")  ytitle("Density")
    if ("`graphname'"!="") {
